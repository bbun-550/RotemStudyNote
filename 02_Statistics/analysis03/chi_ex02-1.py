...
try:
    with conn:
        ...
        sql="""
            select jikwonjik,jikwonpay, jikwonibsail
            from jikwon
        """
        cursor.execute(sql)
        ...
        df['연봉범위'] = pd.cut(df['연봉'], bins=bins, right=True)
        df = df.replace({'이사':1, '부장':2, '과장':3, '대리':4, '사원':5})
        df = df.dropna().reset_index(drop=True)
        print(df.head(3))

        df_ctab = pd.crosstab(index=df['직급'], columns=df['연봉범위'])
        print(df_ctab)
        ...
        print('===========새로운 가설============')
        # 새로운 가설
        # 귀무가설 : 근무년수와 연봉은 연관성이 없다.
        # 대립가설 : 근무년수와 연봉은 연관성이 있다.

        today = datetime.now().year        
        df['근무년수'] = pd.to_datetime(df['입사일'])
        df['근무년수'] = today - df['근무년수'].dt.year
        
        # print(df['근무년수'].unique()) # [15 11  8  6 16 14 12  9 19 10 13]
        year_bins = np.arange(5,20,3)
        df['근속범위'] = pd.cut(df['근무년수'], bins=year_bins, right=True)
        print(df.head(3))

        year_ctab = pd.crosstab(index=df['연봉범위'], columns=df['근속범위'])
        print(year_ctab)

        chi2, pvalue, dof, _ = stats.chi2_contingency(year_ctab)
        print(f'chi2 : {chi2:.4f}\npvalue : {pvalue:.10f}\ndof : {dof}')
