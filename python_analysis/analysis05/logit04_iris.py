...
# 모델 저장
'''
pickle.dump(model, open('python_analysis/analysis05/logimodel.sav','wb'))
del model
'''
read_model = pickle.load(open('python_analysis/analysis05/logimodel.sav','rb'))

# 새로운 값으로 예측 : petal.length / petal.width 만 참여할 수 있다
# print(x_test[:3])
new_data = np.array([[5.1, 1.1],[1.1, 1.1],[6.1, 7.1]])

# 참고 : 만약 표준화된 데이터를 생성했다면 새로운 데이터도 표준화해야 한다.
# - sc.fit(new_data); new_data = sc.tranform(new_data)

new_pred = read_model.predict(new_data) # 내부적으로 softmax가 출려한 값을 argmax로 처리한 것이다.
print(f'예측 결과 : {new_pred}') # [2 0 2]
print(read_model.predict_proba(new_data)) # softmax가 출력한 값은 이 값이다. 하지만 predict로 0, 1, 2 로 분류해준다.
