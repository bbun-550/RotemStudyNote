...
new_pred = model.predict(new_data) # 내부적으로 softmax가 출려한 값을 argmax로 처리한 것이다.
print(f'예측 결과 : {new_pred}') # [2 0 2]
print(model.predict_proba(new_data)) # softmax가 출력한 값은 이 값이다. 하지만 predict로 0, 1, 2 로 분류해준다.
